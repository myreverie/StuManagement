...
void DeleteHash1(HashTable* H ,int &c)
{
	RE str;
	printf("\n请输入要删除的记录的学生姓名：\n");
	scanf("%s",str);
...
void DeleteHash2(HashTable* H ,int &c)
{
	RE idn;
	printf("\n请输入要删除的记录的学生ID：\n");
	scanf("%s",idn);
...
void HashInsert(HashTable* H ,int &c,Record* a) 
{
	NUM_BER++; 
	printf("请输入需要插入的学生姓名：\n",NUM_BER);
...
int main(int argc,char* argv[]){
	...
	while(1) {
		...
		case 3:
			...
			break;
		case 4:
			CreateHash2(H,a);  	     //以学生ID建表
			break;
		case 5:
			c=0;
			SearchHash1(H,c);              //以姓名来查找； 
			break; 
		case 6: 
			c=0;
			SearchHash2(H,c);              //以学生ID来查找； 
			break; 
		case 7:
			c=0;
			DeleteHash1(H,c);              //以姓名来删除； 
			break;			
		case 8:  
			c=0;
			DeleteHash2(H,c);              //以姓名来删除； 
			break;
		case 9: 
		    c=0;
		    HashInsert(H ,c,a) ;
